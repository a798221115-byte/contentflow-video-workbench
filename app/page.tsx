...
export default function Home() {
  ...
  // Initial remote workspace hydration; updates happen after the async boundary.
  ...
  return (
    <main className="app-shell">
      ...
      {showCreate && <div className="modal-backdrop"><form className="create-modal create-modal-wide history-intake-modal" onSubmit={createItem}><header><div><span className="section-kicker">HISTORY SKILL · v3.11.0</span><h2>建立一个历史视频项目</h2><p className="modal-intro">可以只给链接，也可以直接粘贴完整口播稿；两种输入会走不同的证据链。</p></div><button type="button" onClick={()=>setShowCreate(false)}>×</button></header>
        ...
        <label>项目标题<input name="title" required placeholder="识别到书名后，项目会以书名为主标题" defaultValue={extractResult?.title ?? ""} key={extractResult?.title}/></label>
      ...
      {accountEditor && <div className="modal-backdrop"><form className="create-modal account-modal" onSubmit={saveAccount}><header><div><span className="section-kicker">ACCOUNT</span><h2>{accountEditor === "new" ? "添加一个父级账号" : "管理账号资料"}</h2><p className="modal-intro">账号是父级；选题、脚本、成片和数据都归在它下面。</p></div><button type="button" onClick={() => setAccountEditor(null)}>×</button></header>
